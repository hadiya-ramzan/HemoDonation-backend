...
BEGIN
  IF NOT EXISTS (
    ...
    SET @ddl = CONCAT('ALTER TABLE notifications ADD COLUMN `', REPLACE(p_column_name, '`', '``'), '` ', p_column_definition);
...
SELECT
  'Patch 09B applied successfully' AS status,
  ...
  EXISTS (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'notifications'
      AND COLUMN_NAME = 'priority_score'
  ) AS priority_score_column_exists,
  EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'notifications'
      AND INDEX_NAME = 'idx_notifications_priority'
  ) AS priority_index_exists;
